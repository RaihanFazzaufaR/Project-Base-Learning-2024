                    ...
                    <tr class="bg-white border-b text-sm font-medium text-[#7F7F7F] dark:bg-[#2F363E] dark:text-white dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td>
                            <div class="px-6 py-4 flex gap-4 justify-center items-center h-full">
                                ...
                                <form action="{{ route('destroyAkun-level', $userData->username) }}" method="POST" id="deleteForm-{{ $userData->username }}">
                                    @csrf
                                    {!! method_field('DELETE') !!}

                                    <button type="button" onclick="showConfirm('{{ $userData->username }}')" class="flex justify-center items-center gap-2 w-fit text-white bg-[#FF5E5E] rounded-full sm:rounded-lg shadow-xl font-bold h-full sm:px-3 px-2 py-2 hover:bg-[#B34242] hover:scale-105 transition-all">
